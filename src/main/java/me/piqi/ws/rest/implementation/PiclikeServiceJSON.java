...
public class PiclikeServiceJSON implements IEntityService {
	...
	public Response getObject(@PathParam("id") String sid) {
		...
		Like pic = likeDAO.getObject(id, "Piclike");
		if (pic != null) {
	...
	public Response removeObject(@PathParam("id") String sid) {
		Long id = Long.parseLong(sid);
		if (likeDAO.removeObject(id, "Piclike")) {
	...
	public Response createObject(Entity lk) {
		Like newLk = likeDAO.createObject(lk, "Piclike");
		if (newLk != null) {
	...
	public Response updateObject(Entity lk) {
		Like updLk = likeDAO.updateObject(lk, "Piclike");
		if (updLk != null) {
