...
public class PhotoServiceJSON implements IEntityService {
	...
	public Response getObject(@PathParam("id") String sid) {
		...
		Picture pic = pictureDAO.getObject(id, "uPhoto");
		if (pic != null) {
	...
	public Response removeObject(@PathParam("id") String sid) {
		Long id = Long.parseLong(sid);
		if (pictureDAO.removeObject(id, "uPhoto")) {
	...
	public Response updateObject(Entity pic) {
		Picture updPic = pictureDAO.updateObject(pic, "uPhoto");
		if (updPic != null) {
