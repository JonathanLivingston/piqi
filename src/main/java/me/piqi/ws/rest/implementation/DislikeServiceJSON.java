...
public class DislikeServiceJSON implements IEntityService {
	...
	public Response getObject(@PathParam("id") String sid) {
		...
		Like pic = likeDAO.getObject(id, "Dislike");
		if (pic != null) {
	...
	public Response removeObject(@PathParam("id") String sid) {
		Long id = Long.parseLong(sid);
		if (likeDAO.removeObject(id, "Dislike")) {
	...
	public Response createObject(Entity lk) {
		Like newLk = likeDAO.createObject(lk, "Dislike");
		if (newLk != null) {
	...
	public Response updateObject(Entity lk) {
		Like updLk = likeDAO.updateObject(lk, "Dislike");
		if (updLk != null) {
